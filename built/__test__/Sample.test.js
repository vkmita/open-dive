...
describe('Sample', function () {
  ...
  describe('constructor', function () {
    describe('when time === 0', function () {
      it('initializes tissues saturated on surface', function () {
        var strings = Object.values(sample.tissues).map(function (t) {
          return t.n2.gasCompartment.compartment + ' ' + t.n2.pressure;
        });
  ...
  describe('createNextSample', function () {
    var nextSample = sample.createNextSample({ depth: 40, intervalTime: 2 });
    it('creates the next sample with sample tissues', function () {
      var strings = Object.values(nextSample.tissues).map(function (t) {
        return t.n2.gasCompartment.compartment + ' ' + t.n2.pressure;
      });
      expect(strings).toEqual([
    ...
    describe('with a gasSwitch', function () {
      var nitrox = new GasMix_1.default({ o2: 0.32 });
      var gasSwitchSample = new Sample_1.default({
        depth: 0,
        gasMix: GasMix_1.AIR,
        time: 0,
        gasSwitch: nitrox,
      ...
      var nextSample = gasSwitchSample.createNextSample({
        depth: 40,
        intervalTime: 2,
      });
      it('uses the switched gasMix when creating next sample', function () {
        var strings = Object.values(nextSample.tissues).map(function (t) {
          return t.n2.gasCompartment.compartment + ' ' + t.n2.pressure;
        });
        expect(strings).toEqual([
  ...
  describe('stopTime', function () {
    ...
    it('calculates an exact stopTime for each deco step', function () {
      var nextSample = sample.createNextSample({ depth: 40, intervalTime: 2 });
